...
const Quiz = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch('http://localhost:8080/quiz/range?startId=31&endId=45');
  ...
  const submitScore = async (userId, finalScore) => {
    try {
      const response = await fetch(`http://localhost:8080/quiz/updateMultiplicationScore?userId=${userId}&newScore=${finalScore}`, {
        method: 'POST',
        headers: {
