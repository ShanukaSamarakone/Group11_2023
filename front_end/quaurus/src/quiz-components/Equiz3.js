...
const Quiz = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch('http://localhost:8080/quiz/englishrange?startId=31&endId=45');
  ...
  const submitScore = async (userId, finalScore) => { 
    try {
      const response = await fetch(`http://localhost:8080/quiz/updateQuestionmarkScore?userId=${userId}&newScore=${finalScore}`, {
        method: 'POST',
        headers: {
    ...
    return (
      <section className="page score" style={{ display: 'block' }}>
        <div className="content">
          ...
          <button className="Return" onClick={handleReturnClick}>Return</button>
        </div>
      </section>
    );
  }
  ...
  return (
    <main className="main" >
      <section className="page home">
        ...
        {questions.length > 0 ? questions[currentQuestionIndex].questionText : 'Loading...'}
        </h1>
          <div>
            <input
              type="text"
