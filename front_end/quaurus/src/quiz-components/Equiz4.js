...
const Quiz = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch('http://localhost:8080/quiz/englishrange?startId=46&endId=60');
  ...
  const submitScore = async (userId, finalScore) => { 
    try {
      const response = await fetch(`http://localhost:8080/quiz/updateExclamationScore?userId=${userId}&newScore=${finalScore}`, {
        method: 'POST',
        headers: {
