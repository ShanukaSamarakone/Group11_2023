...
const Quiz = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch('http://localhost:8080/quiz/englishrange?startId=16&endId=30');
  ...
  const submitScore = async (userId, finalScore) => { 
    try {
      const response = await fetch(`http://localhost:8080/quiz/updateCommaScore?userId=${userId}&newScore=${finalScore}`, {
        method: 'POST',
        headers: {
