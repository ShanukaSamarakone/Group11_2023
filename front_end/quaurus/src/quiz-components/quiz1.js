...
const Quiz = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch('http://localhost:8080/quiz/range?startId=1&endId=15');
  ...
  const submitScore = async (userId, finalScore) => {
    try {
      const response = await fetch(`http://localhost:8080/quiz/updateAdditionScore?userId=${userId}&newScore=${finalScore}`, {
        method: 'POST',
        headers: {
