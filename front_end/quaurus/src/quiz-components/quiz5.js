...
const Quiz = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch('http://localhost:8080/quiz/range?startId=61&endId=75');
  ...
  const submitScore = async (userId, finalScore) => {
    try {
      const response = await fetch(`http://localhost:8080/quiz/updatePercentageScore?userId=${userId}&newScore=${finalScore}`, {
        method: 'POST',
        headers: {
